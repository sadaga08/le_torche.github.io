...
const SoftSkils = () => {
   const [articles, setArticles] = useState([]);
     ...
     const featureArticleSoftSkils = articles[0];
     const autreArticleSoftSkils = articles.slice(1);
 
     const imageUrl = featureArticleSoftSkils.image
         ? `${import.meta.env.VITE_API_URL}/uploads/images/${featureArticleSoftSkils.image}`
         : '/placeholder.jpg';
            ...
            <div className="border-2 border-teal-200 w-full rounded-2xl md:rounded-3xl  bg-gradient-to-br from-blue-800 via-cyan-500 to-sky-500 shadow-2xl shadow-slate-400 p-4 md:p-8 lg:p-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                    ...
                    <div className="rounded-3xl bg-black/50 p-4 overflow-hidden">
                        <img
                            src={imageUrl}
                            alt={featureArticleSoftSkils.titre}
                    ...
                    <div className="space-y-6">
                        <h1 className='text-4xl font-extrabold text-slate-100 leading-tight'>
                            {featureArticleSoftSkils.titre}
                        </h1>
                        <p className='text-lg text-slate-200 line-clamp-4'>
                            {featureArticleSoftSkils.content}
                        </p>
                        <span className='text-sm text-slate-300'>
                            Publié le : {new Date(featureArticleSoftSkils.date_publication).toLocaleDateString('fr-FR')}
                        </span>
                        <div className='flex items-center justify-center mt-14'>
                            <Link
                                to={`/Technologie/${featureArticleSoftSkils.id}`}
