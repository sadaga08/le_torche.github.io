...
const Article = () => {
   ...
   const [articles, setArticles] = useState([]);
     ...
     useEffect(() => {
         getArticles()
             .then(data => {
                 ...
                 const articleArticles = data.filter(a =>
                     a.category?.toLowerCase() === 'article'
                 );
                 setArticles(articleArticles);
             })
     ...
     const featureArticleArticles = articles[0];
     const autreArticleArticles = articles.slice(1);
 
     const imageUrl = featureArticleArticles.image
         ? `${import.meta.env.VITE_API_URL}/uploads/images/${featureArticleArticles.image}`
         : '/placeholder.jpg';
     ...
     <>
            ...
            <div className='w-full px-4 py-12'>
                <motion.h1
                    ...
                    className='font-extrabold text-3xl text-blue-700 text-center'
                >
                    Découvrez tous les articles et vidéos sur l'avancée du monde universitaire
                </motion.h1>
            </div>
            ...
            <div className="border-2 border-teal-200 w-full rounded-2xl md:rounded-3xl  bg-gradient-to-br from-blue-800 via-cyan-500 to-sky-500 shadow-2xl shadow-slate-400 p-4 md:p-8 lg:p-10">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
                    ...
                    <div className="rounded-3xl bg-black/50 p-4 overflow-hidden">
                        <img
                            src={imageUrl}
                            alt={featureArticleArticles.titre}
                    ...
                    <div className="space-y-6">
                        <h1 className='text-4xl font-extrabold text-slate-100 leading-tight'>
                            {featureArticleArticles.titre}
                        </h1>
                        <p className='text-lg text-slate-200 line-clamp-4'>
                            {featureArticleArticles.content}
                        </p>
                        <span className='text-sm text-slate-300'>
                            Publié le : {new Date(featureArticleArticles.date_publication).toLocaleDateString('fr-FR')}
                        </span>
                        <div className='flex items-center justify-center mt-14'>
                            <Link
                                to={`/Technologie/${featureArticleArticles.id}`}
